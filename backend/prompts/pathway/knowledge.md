# Germany Care Path Program — Knowledge Base

## Audience
Kenyan healthcare professionals: Nurses (KRCHN/KRN/KRM), Certified Nursing Assistants (CNAs), Clinical Officers, and Health Support Workers seeking employment in Germany.

## Eligibility — Education
- German Language certificate, OR
- KCSE certificate
- All qualifications must be from recognized institutions

## Eligibility — Experience
- No prior experience required but an added advantage if PRESENT

## German language
- A1/A2 to start the program and begin language training
- B1/B2 required before relocation to Germany

## Program timeline (approximate)
1. Preparation (1–2 months): application, credential assessment
23. Placement (1–2 months): school matching, contract, visa processing
4. Relocation (1 month): pre-departure briefing and settlement support in Germany

## Investment / cost (EUR)
- Application & Orientation: €50
- Administration & Management: €150
- Credential Recognition: €200
- Schorlaship Matching : €700
- Visa Support: €250
- Relocation Support: €200
- **Total program cost: €2,300**
- Flexible payment plans available in KES equivalent

## Benefits in Germany
- Starting salaries approximately €2,300–€3,000/month for nurses with recognized qualifications
- Relocation support: housing, bank accounts, integration
- Legal migration compliant with German immigration and employment law

## Next steps
- Eligibility check: /eligibility
- Scholarships (university study funding): /scholarships — NOT covered by this pathway bot
- Contact: (+254) 718-266-072, info@germannursingpathway.com
